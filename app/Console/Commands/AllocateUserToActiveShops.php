...
class AllocateUserToActiveShops extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'user:allocate-active-shops 
                            {user_emails* : list of user email} 
                            {--permission= : permission code for user. In case of multiple permissions, use , to separate.} 
     ...
     * @var string
     */
    protected $description = 'Allocate user to all active shops with some advanced options';
    ...
    public function handle()
    {
        ...
        $permission = array_unique(explode(',', $permission)); // array
        $full_permission = Permission::query()
        ...
        foreach ($permission as $per) {
            if (!in_array($per, $full_permission)) {
                $wrong_permission[] = $per;
            }
        }
        if (!empty($wrong_permission)) {
            $this->error('Permission contains some wrong value, please correct: ['.implode(',', $wrong_permission).']');
            return 0;
        }
        if (is_null($include_shop_eids)) {
            $this->error('shop_eids must have value');
            return 0;
        }
        ...
        //    1. shops inactive
        //    2. shops which doesn't have shop_user
        //    3. shops which doesn't have admin
        //    4. shops belong to more than 1 company
        $inactive_shops = Shops::query()
            ->select('shops_eid')
            ->whereIn('shops_eid', $include_shop_eids)
            ->where('shops_is_active', false)
            ->pluck('shops_eid');
        $this->info('List of shops inactive: ['.$inactive_shops->implode(',').']');
        ...
        $not_admin_shop = ShopUser::query()
            ->select('fk_shop')
            ->distinct()
            ->whereIn('fk_shop', function ($query1) use ($include_shop_eids) {
        ...
        $exclude_shop_eids = array_unique(array_merge($inactive_shops->all(), $not_shop_user->all(), $not_admin_shop->all(), $more_one_company->all()));
        ...
        if (!empty($exclude_shop_eids)) {
            $active_shops->whereNotIn('shops_eid', $exclude_shop_eids);
        }
        $active_shops = $active_shops->get();

        $this->info('List of active shops to apply after exclude ('.$active_shops->count().')');
        if ($active_shops->count() == 0) {
            $this->info('Total shops to apply is 0, nothing to do.');
            return 0;
        }
        $this->table(
        ...
        // check user email must in the company or partnership with company owns list shop
        $user_companies = Users::query()
        ...
        $user_partner_companies = Users::query()
            ->join('company_register_code', 'company_register_code_id', '=', 'fk_company_register_code')
            ->join('companies', 'companies_id', '=', 'fk_company')
            ->join('partnership', 'partnership_from', '=', 'companies_id')
            ->whereIn('users_email', $user_emails)
            ->select('partnership_to')
            ->distinct()
            ->pluck('partnership_to');
        
        $user_all_companies = $user_companies->concat($user_partner_companies->all());
        
        $shop_companies =  Shops::query()
            ->whereIn('shops_eid', $active_shops->pluck('shops_eid'))
            ->join('shop_user', function ($query) {
                $query->on('fk_shop', '=', 'shops_eid');
                $query->on('shop_user_role', '=', \DB::raw('\'admin\''));
            })
        ...
        if ($shop_companies->diff($user_all_companies)->count() > 0) {
            $user_companies = $user_companies->concat($shop_companies->all());
            $this->error('User doesn\'t belong to company or partnership of shops to apply. Please set partnership for these companies mutually: ['.implode(',', $user_companies->all()).']');
            return 0;
        }
        ...
        $common_eids = $active_shops->pluck('shops_eid');
        ...
        $this->info('List shops will be allocated to user ('.$active_shops->count().')');
        $this->table(
            ...
            foreach ($features as $feature) {
                ...
                if (!array_key_exists($data[2], $permission_list)) {
                    $query = Permission::query()
                        ...
                        ->whereIn('permission_code', $permission);
        ...
        \DB::transaction(function () use ($user_emails, $overwrite_normal_role, $permission_list, $shop_admin_feature,
